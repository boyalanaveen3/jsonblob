...
import { FileJson, Mail, Lock, User, ArrowLeft, Sun, Moon, Sparkles, Database, Code, Check } from "lucide-react";
...
const SLIDES = [
  {
    icon: Database,
    title: "Secure JSON Storage",
    description: "Instantly create, persist, and retrieve JSON blobs at the Edge using Cloudflare D1.",
    image: "/auth_illustration.png",
  },
  {
    icon: Code,
    title: "Powerful Editor Tools",
    description: "Format, validate, trace diffs, and inspect collapsible tree views in a single dashboard.",
    image: "/auth_illustration_tools.png",
  },
  {
    icon: Sparkles,
    title: "Edge-Optimized Performance",
    description: "Experience sub-millisecond response times with global CDN replication and Edge Workers.",
  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground transition-colors duration-150 relative overflow-hidden">
      ...
      <section className="hidden lg:flex lg:w-[55%] xl:w-[60%] bg-[#080B11] text-white flex-col justify-between p-12 relative overflow-hidden border-r border-border">
        {/* Dynamic Sliding Background Images with Cross-fade */}
        ...
        <div className="relative z-10 flex items-center gap-2 select-none">
          <div className="relative flex items-center justify-center w-9 h-9 rounded-xl bg-gradient-to-tr from-cyan-500 to-indigo-600 shadow-md shadow-cyan-500/20 text-white font-extrabold text-lg tracking-tighter">
            {"{"}
            <span className="text-[10px] text-cyan-200 absolute mt-0.5 font-semibold">JS</span>
            {"}"}
          </div>
          <span className="font-bold text-base tracking-tight text-white/95">
            JSON<span className="text-cyan-400">Blob</span>
            <span className="text-[9px] font-semibold text-cyan-300 ml-1.5 border border-cyan-500/35 px-1.5 py-0.5 rounded bg-cyan-500/10">SaaS</span>
          </span>
        </div>
      ...
      <section className="flex-1 flex flex-col justify-between p-6 sm:p-12 relative z-10">
          ...
          <div className="bg-card/70 backdrop-blur-md border border-border rounded-2xl shadow-2xl p-6 sm:p-8 overflow-hidden">
            ...
            <div className="flex flex-col items-center gap-2 mb-6">
              <div className="relative flex items-center justify-center w-12 h-12 rounded-2xl bg-gradient-to-tr from-primary to-indigo-650 shadow-lg shadow-primary/25 text-primary-foreground font-extrabold text-2xl tracking-tighter">
                {"{"}
                <span className="text-xs text-primary-foreground/90 absolute mt-0.5 font-semibold">JS</span>
                {"}"}
              </div>
              <h2 className="text-xl font-bold tracking-tight mt-2">
                {isLogin ? "Sign In to JSON Blob" : "Create your Account"}
              </h2>
              <p className="text-xs text-muted-foreground text-center max-w-[280px]">
                {isLogin
                  ? "Access your saved JSON blobs and workspaces"
                  : "Register to save, share, and query JSON datasets"}
              </p>
            </div>
        ...
        <footer className="text-center text-[10px] text-muted-foreground w-full">
          &copy; {new Date().getFullYear()} JSON Blob SaaS. All rights reserved.
        </footer>
      </section>
    </div>
  );
