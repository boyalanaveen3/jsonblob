...
async function run() {
  ...
  try {
    ...
    await page.evaluate(() => {
      window.currentEditor.setValue('{"a":1,"b":2}');
    });
    await delay(200);

    const formatBtn = page.locator("button[title='Format JSON']");
    await formatBtn.click();
    await page.waitForSelector(':has-text("Formatted successfully")');
    ...
    await page.evaluate(() => {
      window.currentEditor.setValue('{"invalid": json');
    });
    await delay(200);

    const validateBtn = page.locator("button[title='Validate JSON syntax']");
    await validateBtn.click();
    await page.waitForSelector(':has-text("JSON invalid")');
    ...
    await page.evaluate(() => {
      window.currentEditor.setValue('{"playwright": true, "status": "active", "value": 100}');
    });
    await delay(200);
    await validateBtn.click();
    await page.waitForSelector(':has-text("Valid JSON syntax")');
    ...
    await copyBtn.click();
    await page.waitForSelector(':has-text("Copied to clipboard")');
    ...
    await page.waitForSelector(':has-text("Downloaded JSON file")');
    ...
    await page.evaluate(() => {
      const val = window.currentEditor.getValue();
      if (val.trim().endsWith("}")) {
        window.currentEditor.setValue(val.substring(0, val.lastIndexOf("}")) + ', "additional_garbage": 123}');
      } else {
        window.currentEditor.setValue('{"playwright": true, "status": "active", "value": 100, "additional_garbage": 123}');
      }
    });
    await delay(200);
    ...
    await page.waitForSelector(':has-text("Workspace reset")');
    ...
    await titleInput.fill("Playwright Full Featured Blob", { force: true });

    const saveBtn = page.locator("header button:has-text('Save')");
    await saveBtn.click();
    await page.waitForSelector(':has-text("Blob created successfully")');
    ...
    await page.evaluate(() => {
      const val = window.currentEditor.getValue();
      if (val.trim().endsWith("}")) {
        window.currentEditor.setValue(val.substring(0, val.lastIndexOf("}")) + ', "autosaved": true}');
      }
    });
    ...
    await titleInput.fill("Playwright Full Featured Blob (Updated)", { force: true });

    await saveBtn.click();
    await page.waitForSelector(':has-text("Blob updated successfully")');
    ...
    await page.waitForSelector(':has-text("Blob deleted successfully")');
    await delay(1000);
